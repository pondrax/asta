...
export const checkUser = command(
  type({
    email: "string.email?",
    nik: "string?",
  }),
...
export const verifyTurnstile = command(
  type({
    __token: "string",
  }),
  async (props) => {
...
export const signDocument = command(
  type({
    // __token: 'string',
    __manual: "boolean?",
    __asDraft: "boolean?",
    ...
    try {
      ...
      if (response.data.file && response.data.file.length > 0) {
        ...
        const saved = await storage.save(
          `documents/${props.__asDraft ? "draft_" : "signed_"}${props.fileName}`,
          buffer,
        );
        ...
        if (saved.url) {
          ...
          await db.query.documents.upsert({
            data: {
              ...
              to: props.to,
              signatureProperties: props.__asDraft
          ...
          const id =
            (
              await db.query.documentStatistics.findFirst({
                where: {
                  type: "signed",
                  created: {
...
export const verifyDocument = command(
  type({
    file: "string",
  }),
  async (props) => {
